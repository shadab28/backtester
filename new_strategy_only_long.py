"""
Intraday EMA-RSI Stock Strategy Backtester (LONG ONLY)
...
Entry Rules:
- LONG ONLY: EMA(3) > EMA(10) AND RSI(14) > 60 AND Close > EMA(50) -> Buy at high of entry candle
...
TRADE_LOG_DIR = SCRIPT_DIR / "trade_logs_only_long"
...
class Trade:
    ...
    def __init__(self, symbol: str, side: str, entry_time: datetime, entry_price: float, size: int):
        self.symbol = symbol
        self.side = side  # 'LONG' only in this version
...
def simulate_stock_trading(symbol: str, df: pd.DataFrame, date: datetime.date, 
    ...
    """
    Simulate LONG ONLY trading for a single stock on a given date
    ...
    # Keep full day data for lookback
    ...
    for i in range(len(df_day)):
        ...
        if current_trade is None and not entered_today:
            ...
            # LONG Entry ONLY: EMA(3) > EMA(10) AND RSI(14) > 60 AND Close > EMA(50)
...
def run_backtest(start_date: str, end_date: str = None, use_multiprocessing: bool = True, 
    ...
    print(f"\n{'='*60}")
    print(f"INTRADAY EMA-RSI STRATEGY BACKTEST (LONG ONLY)")
    print(f"{'='*60}")
    ...
    # Print results
    print(f"\n{'='*60}")
    print("PERFORMANCE METRICS (LONG ONLY)")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Intraday EMA-RSI Stock Strategy Backtester (LONG ONLY)",
...
Examples:
  ...
  python new_strategy_only_long.py --start-date 2025-08-01 --end-date 2025-08-31
  python new_strategy_only_long.py --all
  python new_strategy_only_long.py --start-date 08-01-2025  # MM-DD-YYYY format also supported
        """
    )
